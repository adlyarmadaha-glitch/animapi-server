...
console.log('[Admin] Config file:', configFile);
console.log('[Admin] Exists:', fs.existsSync(configFile));

function loadConfig() {
  try {
    const raw = fs.readFileSync(configFile, 'utf-8');
    console.log('[Admin] Raw config:', raw);
...
adminRouter.get('/ban', (req, res) => { if (!isAdmin(req)) return res.redirect('/admin/login'); const ip = (req.query.ip||'').trim(); if (ip) { const cfg = loadConfig(); if (!cfg.bannedIPs.includes(ip)) { cfg.bannedIPs.push(ip); saveConfig(cfg); } } res.redirect('/admin'); });
adminRouter.get('/unban', (req, res) => { if (!isAdmin(req)) return res.redirect('/admin/login'); const ip = (req.query.ip||'').trim(); if (ip) { const cfg = loadConfig(); cfg.bannedIPs = cfg.bannedIPs.filter((i: string) => i !== ip); saveConfig(cfg); } res.redirect('/admin'); });
