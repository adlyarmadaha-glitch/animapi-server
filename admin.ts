...
console.log('[Admin] DATA_DIR:', DATA_DIR);
console.log('[Admin] configFile:', configFile);
console.log('[Admin] configFile exists:', fs.existsSync(configFile));

function loadConfig() {
  try {
    const raw = fs.readFileSync(configFile, 'utf-8');
    console.log('[Admin] Config loaded:', raw);
    return JSON.parse(raw);
  } catch(e) {
    console.error('[Admin] Error loading config:', e);
    return { adminUser: 'admin', adminPass: 'animapi2025', bannedIPs: [], adminKey: 'animapi-admin-secret' };
  }
}

function isAdmin(req: any): boolean {
  const cfg = loadConfig();
  console.log('[Admin] Checking auth:', req.cookies?.adminAuth, 'Expected:', cfg.adminKey);
  return req.cookies?.adminAuth === cfg.adminKey;
}

const adminRouter = express.Router();

adminRouter.get('/login', (req, res) => {
  res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Login Admin</title>
<style>*{margin:0;padding:0;box-sizing:border-box}body{background:#06060f;display:flex;justify-content:center;align-items:center;min-height:100vh;font-family:system-ui}.box{background:#0a0a1a;border:1px solid #2a2a4a;border-radius:16px;padding:40px;width:380px;text-align:center}.box h1{font-size:2em;background:linear-gradient(90deg,#e94560,#a855f7);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:20px}input{width:100%;padding:12px;margin-bottom:12px;border:1px solid #2a2a4a;border-radius:8px;background:#0a0a15;color:#e0e0e0}button{width:100%;padding:12px;background:#a855f7;color:#fff;border:none;border-radius:8px;font-weight:600;cursor:pointer}button:hover{background:#9333ea}.err{color:#ef4444;margin-bottom:10px}</style></head><body><div class="box"><h1>🛡️ Admin</h1>${req.query.error ? '<div class="err">' + req.query.error + '</div>' : ''}<form action="/admin/login" method="POST"><input name="username" placeholder="Username" required><input name="password" type="password" placeholder="Password" required><button>Login</button></form></div></body></html>`);
});
...
adminRouter.post('/login', (req, res) => {
  ...
  console.log('[Admin] Expected:', config.adminUser, config.adminPass);
