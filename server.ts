...
import { Otakudesu, Animasu } from "./index.js";
...
<div class="hero"><h1>🎌 Anime REST API</h1><p>REST API Anime Streaming Indonesia — Otakudesu + Animasu</p></div>
<div class="container">
<div class="base-url">Base URL: ${req.protocol}://${req.get('host')}</div>
<div class="section-title">📺 Utama</div>
<div class="endpoint"><span class="method">GET</span><span class="path">/anime/home</span><div class="desc">Data halaman utama (ongoing + complete dari Otakudesu & Animasu)</div></div>
...
<div class="endpoint"><span class="method">GET</span><span class="path">/anime/ongoing-anime</span><div class="desc">Anime sedang tayang (Otakudesu + Animasu digabung)</div><div class="example">?page=1</div></div>
...
<div class="endpoint"><span class="method">GET</span><span class="path">/anime/genre</span><div class="desc">Semua genre</div></div>
...
<footer>Made with ❤️ | Otakudesu + Animasu</footer>
</body></html>`);
});

// HOME - gabungan otakudesu + animasu
app.get("/anime/home", async (req, res) => {
  try {
    const [ot_ongoing, ot_complete, an_ongoing] = await Promise.all([
    ...
    const allOngoing = [...(ot_ongoing.animes||[]), ...(an_ongoing.animes||[])];
    res.json({ status:"success", data:{
      ongoing: { animeList: formatAnime(allOngoing) },
      completed: { animeList: formatAnime(ot_complete.animes||[]) }
    }});
...
// DETAIL ANIME - coba otakudesu dulu, fallback animasu
...
// EPISODE - coba otakudesu dulu, fallback animasu
app.get("/anime/episode/:slug", async (req, res) => {
  try {
    let data: any;
    try { data = await otakudesu.streams(req.params.slug); }
    catch { data = await animasu.streams(req.params.slug); }
    res.json({ status:"success", data:{
      title: data.title || "",
...
app.get("/anime/schedule", async (req, res) => {
  try {
    const data = await otakudesu.search({ filter: { status: "Ongoing" } } as any);
    const days = ["Senin","Selasa","Rabu","Kamis","Jumat","Sabtu","Minggu"];
    res.json({ status:"success", data: days.map(day => ({ day, anime_list: formatAnime(data.animes||[]).slice(0,5) })) });
  } catch(e:any){ res.status(500).json({ error: e.message }); }
});
...
app.get("/anime/batch/:slug", async (req, res) => {
  try {
    const data = await otakudesu.detail(req.params.slug);
