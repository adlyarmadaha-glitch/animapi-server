...
import { hasNextPageAndGet } from "../../utils/index.js";
import {
...
export class AnimeIndo extends Provider {
  ...
  async search(options?: SearchOptions): Promise<SearchResult> {
    ...
    const res = await this.api.get(
      alphabet ? `${this.baseUrl}/anime-list/` :
      genre ? `${this.baseUrl}/genres/${genre}/` :
      type == "Movie" ? `${this.baseUrl}/movie/page/${page}/` :
      keyword ? `${this.baseUrl}/search/${keyword}/` :
      this.baseUrl
    );
    const $ = this.cheerio.load(res.data);
    let hasNext = $("a").filter((_, el) => $(el).text().replace(/\s+/g, "").trim() === "»").length >= 1;
    ...
    if (alphabet) {
      ...
      $("#content-wrap .menu .anime-list a").each((_, el) => {
        ...
        if ($a.text().toLowerCase().startsWith(alphabet.toLowerCase())) {
          const slug = $a.attr("href")?.trim().split("/anime/")[1]?.replace("/", "").trim();
          if (slug) preSlugs.push(slug);
        }
      });
      ...
      els.each((_, el) => {
        const slug = (keyword || type || genre)
          ? $(el).attr("href")?.trim().split("/anime/")[1]?.replace("/", "").trim()
          : $(el).attr("href")?.trim().split("-episode-")[0]?.replace("/", "").trim();
  ...
  async detail(slug: string): Promise<Anime | undefined> {
    ...
    if (!img?.includes("http")) img = `${this.baseUrl}/${img}`;
    const synopsis = $("#content-wrap .ngirix .menu .detail p").text().trim();
    const genres: Genre[] = [];
    $("#content-wrap .ngirix .menu .detail li a").each((_, el) => {
      const $a = $(el);
      const slug = $a.attr("href")?.trim().split("/genres/")[1]?.replace("/", "");
      genres.push({ name: $a.text().trim(), slug: slug || "", source: this.name });
    });
    const episodes: Episode[] = [];
    $("#content-wrap .ngirix .menu .ep a").each((_, el) => {
      const $a = $(el);
      episodes.push({ name: $a.text(), slug: $a.attr("href")?.replace("/", "").replace("/", "") || "", source: this.name });
    });
    const data: Anime = {
      slug, title, synopsis, posterUrl: img,
      status: "UNKNOWN", genres, episodes,
      studios: [], batches: [], producers: [], characterTypes: [],
      source: this.name,
    };
  ...
  async genres(): Promise<Genre[]> {
    const cached = this.cache.get(`genres-${this.name}`);
    if (cached && this.options.cache) return cached;
    ...
    $(".list-genre a").each((_, el) => {
      const $a = $(el);
      const slug = $a.attr("href")?.split("/genres/")[1]?.replace("/", "").trim();
      genres.push({ name: $a.text().trim(), slug: slug || "", source: this.name });
    });
  ...
  async streams(slug: string): Promise<Stream[]> {
    const cached = this.cache.get(`streams-${this.name}-${slug}`);
    if (cached && this.options.cache) return cached;
    ...
    $("#content-wrap .menu .servers a").each((_, el) => {
      ...
      if (!embedUrl?.includes("http")) embedUrl = `${this.baseUrl}${embedUrl}`;
      streams.push({ name: $a.text(), url: embedUrl, source: this.name });
    });
    if (this.options.cache) this.cache.set(`streams-${this.name}-${slug}`, streams);
    return streams;
  }
}
