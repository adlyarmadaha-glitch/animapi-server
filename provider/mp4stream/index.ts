...
export class MP4Stream extends Provider {
  ...
  async search(options?: SearchOptions): Promise<SearchResult> {
    ...
    const slugs: string[] = [];
    $("article a[href*='/anime/']").each((_, el) => {
  ...
  async detail(slug: string): Promise<Anime | undefined> {
    ...
    // Ambil episode dari halaman berikutnya jika ada pagination
    const totalPages = $(".pagination .page-numbers:not(.next):not(.prev)").length || 1;
  ...
  async streams(slug: string): Promise<Stream[]> {
    ...
    // Cari semua link download/streaming
    ...
    // Jika tidak ketemu, coba ambil semua link dari mirror
    if (streams.length === 0) {
