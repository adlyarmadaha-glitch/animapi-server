...
export async function getNonceCode(api: AxiosStatic, baseUrl: string) {
  const res = await api.post(
    ...
    new URLSearchParams({ action: ACTION_GET_NONCE_CODE }).toString(),
...
export async function getEmbedUrl(api: AxiosStatic, baseUrl: string, nonce: string, id: string, i: string, q: string) {
  const res = await api.post(
    `${baseUrl}/wp-admin/admin-ajax.php`,
    new URLSearchParams({ action: ACTION_GET_EMBED_CODE, nonce, id, i, q }).toString(),
